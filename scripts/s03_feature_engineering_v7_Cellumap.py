...
from utils_feature_engineering_v7_cellUMAP import (
    # Data loading
    ...
    create_features,
    # CL umap
    train_and_apply_umap, plot_CL_umap,
)
...
CL_META_PATH          =  _root / 'data' / 'DepMap' / 'Model.csv'
...
OUT_CL_UMAP        = SAVE_DIR / "CL_based_umap_features.csv"
...
OUT_REDUCER_CL       = SAVE_DIR / "cl_umap_reducer.pkl"
...
# ── 2. Load data ──────────────────────────────────────────────────────────────
print("Loading CL meta data")
cl_meta_data = pd.read_csv(CL_META_PATH)
...
# ── 13. CL features based clustering ──────────────────────────────────────────────────────────────────

CL_umap = train_and_apply_umap(full_df = cluster_out_pd, n_neighbors=3, 
                         min_dist=0.05, n_components=10,
                         exclude_cols = ['ModelID', 'split'],
                         out_path = OUT_REDUCER_CL)



CL_umap_viz = train_and_apply_umap(full_df = cluster_out_pd,
                                   out_path = None,
                                    n_neighbors=3, 
                                    min_dist=0.01, n_components=2,
                                    exclude_cols = ['ModelID', 'split'])

plot_CL_umap(X_umap = CL_umap_viz,
             meta_df = cl_meta_data,
             save_dir = SAVE_DIR,
             color_col = "OncotreePrimaryDisease",
             tag = "")
...
print("\nSaving …")
cluster_out_pd.to_csv(OUT_CLUSTER, index=False)   
data_out_pd.to_csv(OUT_MAIN, index=False)
CL_umap.to_csv(OUT_CL_UMAP, index=False)            

print(f"  → {OUT_CLUSTER}  {cluster_out_pd.shape}")
print(f"  → {OUT_MAIN}  {data_out_pd.shape}")
print(f"  → {OUT_CL_UMAP}  {CL_umap.shape}")
