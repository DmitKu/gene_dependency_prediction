...
from src.utils_RNAbased_crispr_model_v7_2_geneMSE_umap import (
    GeneDataset,
...
MODEL_KWARGS = dict(
    ...
    compress_dim  = 1024,
    dropout       = 0.2,
    # umap_dim is resolved at build time from the dataset (see build_model)
)
...
def build_model(train_ds: GeneDataset) -> CRISPRSensitivityModelV3:
    """
    Construct the model, resolving feature sizes from the dataset so the
    config stays in sync automatically when the HDF5 changes.

    umap_dim is read from the dataset's umap_features tensor, so no manual
    update to MODEL_KWARGS is needed when the UMAP dimensionality changes.
    """
    umap_dim = train_ds.umap_features.shape[1]
    print(f"  -> UMAP dim inferred from dataset: {umap_dim}")
...
def build_optimizer(model: nn.Module, lr: float):
    ...
    # ── UMAP params: moderate LR, low weight decay ───────────────────────────
    # The umap_proj is small and the gate starts near zero, so it benefits from
    # a slightly higher LR than the frozen-style bypass but lower than the trunk.
    umap_params = (
        list(model.cell_tokenizer.umap_proj.parameters())
        + [model.cell_tokenizer.umap_gate]
    )
    ...
    umap_ids          = {id(p) for p in umap_params}
    ...
    optimizer = torch.optim.AdamW(
        [
            ...
            {"params": out_scale_params,     "weight_decay": 0.5,  "lr": lr},
            {"params": umap_params,          "weight_decay": 1e-5, "lr": lr * 0.75},  # ← NEW
        ],
        lr=lr,
        betas=(0.9, 0.999),
    )
...
def train_one_epoch(
    model, loader, optimizer, scheduler, scaler, alpha, device,
    ablate_bypass=False,
):
    model.train()
    ...
    for gene_feat, cell_feat, target, cl_idx, _, _, _, gene_var_w, umap_coords, gene_int_idx in loader:
        ...
        with autocast(device_type=device):
            pred, bypass_reg = model(
                cell_feat, gene_feat,
                ablate_bypass=ablate_bypass,
            )
            loss, mse_term, pearson_r, gdm_mse = combined_loss(
                pred, target, cl_idx,
                gene_idx       = gene_int_idx,     # ← ADD
                alpha          = alpha,
                beta           = 0.4,
...
def main():
    ...
    for epoch in range(start_epoch, EPOCHS):
        ...
        # Report learned UMAP gate each epoch for monitoring
