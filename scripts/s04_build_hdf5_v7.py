...
from utils_hdf5_builder_v7 import (
    load_csvs,
...
CL_UMAP_CSV   = "outputs/RNA_fetures/CL_based_umap_features.csv"
...
def main() -> None:
    ...
    # 1. Load data
    df_cl, df_gene, df_cl_umap = load_csvs(_root, CELL_LINE_CSV, GENE_CSV, CL_UMAP_CSV)

    # 2. Identify feature columns
    cl_feat_cols, gene_feat_cols, cl_umap_feat_cols = identify_feature_columns(df_cl, 
                                                                               df_gene,
                                                                               df_cl_umap)

    # 3. Validate ModelID consistency
    validate_model_ids(df_cl, df_gene, df_cl_umap)
    ...
    cl_feats, gene_feats, cl_umap_feats = load_and_validate_features(
        df_cl, cl_feat_cols,
        df_gene, gene_feat_cols,
        df_cl_umap, cl_umap_feat_cols
    )
    ...
    write_hdf5(
        ...
        cl_umap_feats   = cl_umap_feats,
        crispr_vals     = crispr_vals,
        df_cl           = df_cl,
        df_gene         = df_gene,
        df_cl_umap      = df_cl_umap,
        cl_feat_cols    = cl_feat_cols,
        gene_feat_cols  = gene_feat_cols,
        cl_umap_feat_cols= cl_umap_feat_cols,
        train_idx       = train_idx,
        val_idx         = val_idx,
        test_idx        = test_idx,
        train_cls       = train_cls,
