...
from src.utils_RNAbased_crispr_model_v3 import (
    GeneDataset,
...
def train_one_epoch(model, loader, optimizer, scheduler, scaler, alpha, device, ablate_bypass=False):
    ...
    for gene_feat, cell_feat, target, cl_idx, _, _, _ in loader:
        ...
        with autocast(device_type=device):
            pred, bypass_reg = model(cell_feat, gene_feat, cl_idx=cl_idx, ablate_bypass=ablate_bypass)
...
def main():
    ...
    model = CRISPRSensitivityModelV3(
        ...
        n_cell_lines       = train_ds.n_cell_lines,   # ← NEW
        **MODEL_KWARGS,
    ).to(DEVICE)
