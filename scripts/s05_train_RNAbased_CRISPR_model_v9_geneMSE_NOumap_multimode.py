...
from src.utils_RNAbased_crispr_model_v9_geneMSE_NOumap_multimode import (
    GeneDataset,
...
# Weight for auxiliary classification loss (Change 2).
# 0.3 means: total_loss = regression_loss + 0.3 * cls_loss
# Start conservative — the regression task is primary.
CLS_LOSS_WEIGHT = 0.3

# pos_weight for BCE: accounts for ~9:1 class imbalance at p10 threshold.
# Without this BCE trivially predicts all-negative and gets 90% accuracy.
# Formula: (n_negative / n_positive) ≈ 9.0 for p10.
# Recomputed automatically from the training set in main() — this is the fallback.
CLS_POS_WEIGHT_FALLBACK = 9.0
...
def compute_cls_pos_weight(train_ds: GeneDataset, device: str) -> torch.Tensor:
    """
    Compute pos_weight for BCE from the actual training label distribution.
    pos_weight = n_negative / n_positive (exact, not the p10 approximation).
    Returns a scalar tensor on `device`.
    """
    n_pos = train_ds.sensitive_labels.sum().item()
    n_neg = len(train_ds) - n_pos
    if n_pos < 1:
        return torch.tensor(CLS_POS_WEIGHT_FALLBACK, device=device, dtype=torch.float32)
    pw = n_neg / n_pos
    print(f"  -> cls pos_weight = {pw:.2f}  "
...
def build_optimizer(model: nn.Module, lr: float):
    # sensitivity_head grouped with head + cond_proj:
    # lighter weight decay (1e-6) since it's a small auxiliary branch.
    head_and_cond_params = (
        list(model.head.parameters())
        + list(model.cond_proj.parameters())
        + list(model.sensitivity_head.parameters())   # ← Change 2
    )
...
def train_one_epoch(
    ...
    for (gene_feat, cell_feat, target, cl_idx,
        ...
        with autocast(device_type=device):
            pred, bypass_reg, sensitivity_logit = model(
                cell_feat, gene_feat,
                ablate_bypass=ablate_bypass,
            )

            # ── Regression loss (unchanged) ──────────────────────────────────
            reg_loss, mse_term, pearson_r, _ = combined_loss(
                pred, target, cl_idx,
            ...
            # ── Auxiliary classification loss (Change 2) ─────────────────────
            # pos_weight corrects the ~9:1 class imbalance at p10 threshold.
            # Without it, BCE trivially minimises by predicting all-negative.
            cls_loss = F.binary_cross_entropy_with_logits(
                sensitivity_logit.view(-1),
                sensitive_label.view(-1),
                pos_weight=cls_pos_weight,
            )

            loss = reg_loss + bypass_reg + CLS_LOSS_WEIGHT * cls_loss
...
def save_checkpoint(path, epoch, model, optimizer, scheduler, scaler,
    ...
    ckpt = {
        ...
        "loss_config":      {"alpha": alpha, "cls_weight": CLS_LOSS_WEIGHT},
...
def log_epoch(path, epoch, lr,
    ...
    with open(path, "a") as f:
        f.write(
            f"{epoch},{lr:.6e},"
            f"{train_loss:.6f},{train_mse:.6f},{train_pearson:.4f},{train_cls:.4f},"
...
def main():
    ...
    qt = None
    print("qt = None  →  metrics reported in raw Chronos space")

    train_ds, val_ds, train_loader, val_loader = build_loaders(H5_PATH, BATCH_SIZE)

    # ── Compute exact pos_weight from training labels ─────────────────────
    cls_pos_weight = compute_cls_pos_weight(train_ds, DEVICE)
    ...
    for epoch in range(start_epoch, EPOCHS):
        ...
        print(
            ...
            f"cls_loss(train) {train_cls:.4f} | "
            f"cls_auc(val) {val_cls_auc:.4f} | "
        ...
        if (epoch + 1) % 10 == 0:
            ...
            (_, _, _,
             mae_no_bypass, _,
             pearson_no_bypass, pearson_full_demeaned_no_bypass,
             _, _, _) = evaluate(
                model, val_loader, DEVICE,
                qt=qt, alpha=current_alpha,
                ablate_bypass=True,
