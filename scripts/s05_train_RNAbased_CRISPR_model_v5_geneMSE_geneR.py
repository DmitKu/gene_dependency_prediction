...
from src.utils_RNAbased_crispr_model_v5_geneMSE_geneR import (
    GeneDataset,
...
def build_optimizer(model: nn.Module, lr: float):
    ...
    optimizer = torch.optim.AdamW(
        [
            ...
            {"params": out_scale_params,     "weight_decay": 0.0,  "lr": lr}, #V4 was lr * 0.5
        ],
        lr=lr,
        betas=(0.9, 0.999),
    )
    ...
    for gene_feat, cell_feat, target, cl_idx, _, _, _, gene_var_w, gene_idx in loader:
        ...
        with autocast(device_type=device):
            ...
            loss, mse_term, pearson_r, _ = combined_loss(
                pred, target, cl_idx, gene_idx, alpha=alpha, beta=0.4,
        ...
        with torch.no_grad():
            model.out_scale.clamp_(0.1, 10.0) #V4 was 0.5, 5.0
