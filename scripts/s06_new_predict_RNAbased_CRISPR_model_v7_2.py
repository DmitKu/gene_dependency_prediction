...
=====================================
Inference / evaluation entry-point for the CRISPR Sensitivity Model v3 + UMAP.
...
Batch tuple layout (GeneDataset.__getitem__, 10 elements)
----------------------------------------------------------
  ...
  [8] umap_coords     Tensor [U]
  [9] gene_int_idx    LongTensor scalar

model.forward() signature
-------------------------
    pred, bypass_reg = model(cell_feat, gene_feat, umap_coords,
                             ablate_bypass=False)
"""
...
# umap_dim is resolved automatically from the dataset (same as training).
MODEL_KWARGS = dict(
    hidden_dim   = 128,
    gene_hidden  = 64,
    n_attn_slots = 64,
    ...
    compress_dim = 1024,
    dropout      = 0.2,
    # umap_dim injected below from ds.umap_features.shape[1]
)
...
def build_model(ds: GeneDataset) -> CRISPRSensitivityModelV3:
    """
    Construct model with feature sizes and umap_dim inferred from the dataset,
    mirroring the build_model() logic in the training script so dimensions
    always stay in sync with whatever HDF5 was used.
    """
    umap_dim = ds.umap_features.shape[1]
...
def main():
    ...
    # ── Inference ─────────────────────────────────────────────────────────────
    # Unpack all 10 elements of the batch tuple.
    # model.forward() now requires umap_coords as a positional argument.
    all_pred          = []
    all_target        = []
    all_cl_idx        = []
    all_sample_idx    = []
    ...
    with torch.no_grad():
        for (
            ...
            model_id,     # [5]
            sample_idx,   # [6]
            _gene_var_w,  # [7]  not needed for inference
            umap_coords,  # [8]  ← NEW: must be forwarded to model
            _gene_int_idx # [9]  not needed for per-sample collection
        ) in loader:

            gene_feat   = gene_feat.to(DEVICE,   non_blocking=True)
            cell_feat   = cell_feat.to(DEVICE,   non_blocking=True)
            umap_coords = umap_coords.to(DEVICE, non_blocking=True)  # ← NEW

            with autocast(device_type=DEVICE_TYPE):
                pred, _ = model(                 # unpack (output, bypass_reg)
                    cell_feat,
                    gene_feat,
                    umap_coords,                 # ← NEW: required positional arg
    ...
    # evaluate() now unpacks gene_var_weight (pos 7), umap_coords (pos 8), and
    # gene_int_idx (pos 9) internally — no changes needed at the call site.
    (_, _, _,
     mae, rmse,
     pearson_full,
