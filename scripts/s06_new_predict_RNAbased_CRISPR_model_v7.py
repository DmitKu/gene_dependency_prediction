...
def main():
    ...
    # ── Inference ─────────────────────────────────────────────────────────────
    # GeneDataset.__getitem__ returns 8 elements:
    #   [0] gene_feat  [1] cell_feat  [2] target  [3] cl_idx
    #   [4] gene_id    [5] model_id   [6] sample_idx  [7] gene_var_weight
    # model.forward() returns (output [B,1], bypass_reg scalar) — unpack both.
    ...
    with torch.no_grad():
        for (gene_feat, cell_feat, target, cl_idx,
             gene_id, model_id, sample_idx, _) in loader:         # _ = gene_var_weight
    ...
    # evaluate() handles the 8-element batch tuple internally and passes
    # gene_var_weight to combined_loss automatically.  qt=None skips any
    # inverse-transform inside evaluate() as well.
    (_, _, _,
     mae, rmse,
     pearson_full,
