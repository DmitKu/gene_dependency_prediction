# -*- coding: utf-8 -*-
"""
s05_train_RNAbased_CRISPR_model.py  (v4)
========
Training entry-point for CRISPR Sensitivity Model v4.

Changes from v3
---------------
  1. Import sensitivity_weighted_loss instead of combined_loss
  2. model() now returns (point_pred, quantile_preds, bypass_reg) — unpack 3 values
  3. sensitivity_weighted_loss called with (point_pred, quantile_preds, target, cl_idx)
  4. MODEL_KWARGS gains n_moe_experts
  5. diagnose_bypass print updated (router entropy replaces attn entropy)
"""
...
from src.utils_RNAbased_crispr_model_v2 import (
    GeneDataset,
    CRISPRSensitivityModelV3,
    sensitivity_weighted_loss,   # FIX 4: replaces combined_loss
    evaluate,
    diagnose_bypass,
)
...
MODEL_KWARGS = dict(
    ...
    compress_dim   = 1024,
    dropout        = 0.2,
    n_moe_experts  = 4,    # FIX 1: MoE expert count (was not present in v3)
)
...
def build_loaders(h5_path, batch_size):
    ...
    return train_ds, val_ds, DataLoader(train_ds, shuffle=True, **kw), DataLoader(val_ds, shuffle=False, **kw)
...
def build_optimizer(model, lr):
    ...
    head_ids      = {id(p) for p in head_and_cond_params}
...
# ============================================================
# Training step  — FIX: unpack 3 values from model(), use sensitivity_weighted_loss
# ============================================================
...
def train_one_epoch(model, loader, optimizer, scheduler, scaler, alpha, device, ablate_bypass=False):
    ...
    for gene_feat, cell_feat, target, cl_idx, _, _, _ in loader:
        ...
        with autocast(device_type=device):
            # FIX 2: model now returns 3 values — unpack all three
            point_pred, quantile_preds, bypass_reg = model(
                cell_feat, gene_feat, ablate_bypass=ablate_bypass
            )
            # FIX 4: use sensitivity_weighted_loss; pass both point_pred and quantile_preds
            loss, mse_term, pearson_r, _ = sensitivity_weighted_loss(
                point_pred, quantile_preds, target, cl_idx, alpha=alpha, beta=0.4
            )
...
def save_checkpoint(path, epoch, model, optimizer, scheduler, scaler,
    ...
    ckpt = {
        ...
        "best_val_loss": best_val_loss, "best_pearson": best_pearson,
        "pearson": pearson_full, "pearson_demeaned": pearson_full_demeaned,
...
def load_checkpoint(path, model, optimizer, scheduler, scaler, device):
    ...
    return ckpt["epoch"] + 1, ckpt["best_val_loss"], ckpt.get("best_pearson", -1.0), ckpt.get("patience_counter", 0)


def init_log(path, resume):
    with open(path, "a" if resume else "w", encoding="utf-8") as f:
        if not resume:
            f.write(
                "epoch,lr,"
...
def log_epoch(path, epoch, lr,
    ...
    with open(path, "a", encoding="utf-8") as f:
        f.write(
            ...
            f"{cl_t_mean:.4f},{cl_t_std:.4f},{epoch_time:.2f}\n"
        )


def get_dynamic_alpha(epoch, warmup_epochs=15, start_alpha=0.6, end_alpha=0.1):
...
# ============================================================
# Main  (minimal changes — only where model API changed)
# ============================================================

def main():
    ABLATE_BYPASS_EPOCHS = 10

    for path, label in [(H5_PATH, "HDF5"), (TRANSFORMER_PATH, "QuantileTransformer")]:
        if not path.exists():
    ...
    for epoch in range(start_epoch, EPOCHS):
        ...
        current_alpha = get_dynamic_alpha(epoch=epoch, warmup_epochs=40,
                                          start_alpha=0.6, end_alpha=0.2)
        ...
        (val_loss, val_mse, val_pearson,
         ...
         cl_t_mean, cl_t_std) = evaluate(
            model, val_loader, DEVICE, qt=None, alpha=current_alpha,
        )
        ...
        log_epoch(LOG_PATH, epoch + 1, current_lr,
                  train_loss, train_mse, train_pearson,
                  val_loss, val_mse, val_pearson,
                  mae, rmse, pearson_full, pearson_full_demeaned,
                  cl_t_mean, cl_t_std, epoch_time)
        ...
        if (epoch + 1) % 10 == 0:
            ...
            (_, _, _, mae_no_bypass, _, _, pearson_dm_no_bypass, _, _) = evaluate(
                model, val_loader, DEVICE, qt=None,
                alpha=current_alpha, ablate_bypass=True,
            )
            print(
                f"  Trunk-only (no bypass): "
                f"Pearson(demeaned) {pearson_dm_no_bypass:.4f} | MAE {mae_no_bypass:.4f}"
            )
            print(
                f"  Bypass contribution   : "
                f"{pearson_full_demeaned - pearson_dm_no_bypass:+.4f} Pearson points"
            )
            if pearson_dm_no_bypass < 0.1:
                print("  WARNING: trunk learning collapsed -- bypass carrying all signal")
