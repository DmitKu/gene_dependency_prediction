# -*- coding: utf-8 -*-
"""
scripts/train_crispr_v5.py
==========================
Training entry-point for CRISPR Sensitivity Model V5 (hybrid with cluster attention).

Usage
-----
    python scripts/train_crispr_v5.py
"""
...
SAVE_PATH        = _root / "outputs" / "model_training_v5"
...
# Model hyperparameters — V5 uses attention-based architecture
MODEL_KWARGS = dict(
    hidden_dim  = 128,
    n_slots     = 32,
    n_heads     = 8,
...
CHECKPOINT_PATH    = SAVE_PATH / "crispr_v5_checkpoint.pt"
BEST_PEARSON_PATH  = SAVE_PATH / "crispr_v5_best_pearson.pt"
FINAL_WEIGHTS_PATH = SAVE_PATH / "crispr_v5_final.pt"
LOG_PATH           = SAVE_PATH / "training_history_v5.csv"
...
def build_optimizer(model: nn.Module, lr: float) -> torch.optim.Optimizer:
    """
    Parameter groups with differentiated learning rates for V5.

    cluster_emb / cluster_token_emb : lower LR + higher weight decay
        Gene cluster embeddings. Sparse clusters need careful treatment.

    attention layers : lower LR
        Cross-attention is new in V5 — learn slowly so FiLM path establishes first.

    bilinear : lower LR
        Interaction term learns second.

    gene_bias / cell_bias : very low LR
        Main effect terms. Zero-initialised.

    everything else : base LR
        Cell encoder, gene stat encoder, gate, FiLM blocks, head.
    """
    cluster_params = (
        list(model.gene_cluster_emb.parameters()) +
        list(model.cluster_token_emb.parameters())
    )
    attn_params = (
        list(model.cross_attn1.parameters()) +
        list(model.cross_attn2.parameters())
    )
    bilinear_params = (
    ...
    main_eff_params = (
        list(model.gene_bias.parameters()) +
        list(model.cell_bias.parameters())
    )

    cluster_ids = {id(p) for p in cluster_params}
    ...
    return torch.optim.AdamW(
        [
            ...
            {"params": cluster_params,    "lr": lr * 0.3,   "weight_decay": 1e-3},
            {"params": attn_params,       "lr": lr * 0.3,   "weight_decay": 1e-4},
...
def get_dynamic_alpha(
    ...
    start_alpha:   float = 0.7,
    end_alpha:     float = 0.2,
) -> float:
    """
    Cosine decay of MSE weight over warmup_epochs, then holds at end_alpha.

    Early training: high MSE weight keeps gradients stable.
    Later training: lower MSE weight, higher Pearson weight.
    """
...
def main():
    ...
    print(f"\nModel config (V5 - Hybrid with Cluster Attention):")
    print(f"  n_clusters    = {n_clusters}")
    print(f"  gene_feat_size = {gene_feat_size}")
    print(f"  cell_feat_size = {cell_feat_size}")
    print(f"  hidden_dim     = {MODEL_KWARGS['hidden_dim']}")
    print(f"  n_slots        = {MODEL_KWARGS['n_slots']}")
    print(f"  n_heads        = {MODEL_KWARGS['n_heads']}")
    ...
    # ── Training loop ─────────────────────────────────────────────────────
    BETA = 0.4   # demeaned Pearson weight — primary interaction metric
