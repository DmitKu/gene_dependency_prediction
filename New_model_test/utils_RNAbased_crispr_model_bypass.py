...
class CRISPRSensitivityModelV3(nn.Module):
    ...
    def forward(
        ...
        bypass_logit = bypass_logit * bypass_weight   # scaled, never hard-zeroed
        bypass_reg   = bypass_reg   * bypass_weight   # reg scales with contribution
    ...
    with torch.no_grad():
        for gene_feat, cell_feat, target, cl_idx, _, _, _ in loader:
            ...
            with autocast(device_type=device_type):
                pred, _ = model(cell_feat, gene_feat, bypass_weight=bypass_weight)
