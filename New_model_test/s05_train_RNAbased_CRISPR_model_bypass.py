...
from src.utils_RNAbased_crispr_model_bypass import (
    GeneDataset,
...
# ============================================================
def train_one_epoch(model, loader, optimizer, scheduler, scaler, alpha, device, bypass_weight=1.0):
    model.train()
    ...
    for gene_feat, cell_feat, target, cl_idx, _, _, _ in loader:
        ...
        with autocast(device_type=device):
            pred, bypass_reg = model(cell_feat, gene_feat, bypass_weight=bypass_weight)
...
def get_bypass_weight(epoch: int, warmup_epochs: int = 15) -> float:
    """
    Linearly ramp bypass contribution from 0 → 1 over warmup_epochs.
    Bypass parameters receive real (scaled) gradients from epoch 0,
    so they adapt to the residual signal before becoming fully active.
    """
...
def main():
    ...
    for epoch in range(start_epoch, EPOCHS):
        ...
        current_bypass = get_bypass_weight(epoch, warmup_epochs=BYPASS_WARMUP_EPOCHS)
        ...
        if (epoch + 1) % 10 == 0:
            ...
            # ── fast bypass contribution estimate — single batch, not full val pass ──
            _one_batch = next(iter(val_loader))
            _gf  = _one_batch[0][:256].to(DEVICE)
            _cf  = _one_batch[1][:256].to(DEVICE)
            _tgt = _one_batch[2][:256].to(DEVICE)
            _cli = _one_batch[3][:256].to(DEVICE)
            with torch.no_grad():
                _pred_full, _ = model(_cf, _gf, bypass_weight=1.0)
                _pred_none, _ = model(_cf, _gf, bypass_weight=0.0)
                from src.utils_RNAbased_crispr_model import demeaned_pearson
                _r_full = demeaned_pearson(_pred_full, _tgt, _cli).item()
                _r_none = demeaned_pearson(_pred_none, _tgt, _cli).item()
            print(
                f"  Trunk-only (no bypass): "
                f"Pearson(demeaned) {_r_none:.4f}  [single batch estimate]"
            )
            print(
                f"  Bypass contribution   : "
                f"{_r_full - _r_none:+.4f} Pearson points"
            )
            
            if _r_none < 0.1:
                print("  ⚠️  WARNING: trunk learning collapsed — bypass carrying all signal")
            time.sleep(2)   # let DataLoader workers re-arm prefetch after diagnostic stall
