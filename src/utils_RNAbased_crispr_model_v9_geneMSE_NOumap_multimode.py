...
class GeneDataset(Dataset):
    ...
    def __init__(
        ...
        # ── Gene-specific sensitivity labels (Change 2: multi-task) ─────────
    ...
    def __getitem__(self, idx):
        ...
        return (
            ...
            self.sensitive_labels[idx],            # [9] float32 scalar Tensor
        )
...
class CRISPRSensitivityModelV3(nn.Module):
    ...
    def __init__(
        ...
        # ── ⑩ Auxiliary sensitivity classification head (Change 2) ──────────
        # Branches off cell_context2 — the post-attention cell representation.
        # This is the right branch point because:
        #   - gene_emb alone already encodes broad essentiality (not useful)
        ...
        self.sensitivity_head = nn.Sequential(
            ...
            nn.GELU(),
            nn.Dropout(0.1),
            nn.Linear(32, 1),   # raw logit — BCE loss applied in training script
        )
    ...
    def forward(
        ...
        # Returns: (regression_out [B,1], bypass_reg scalar, sensitivity_logit [B,1])
        ...
        # ── ⑧ Auxiliary classification output (Change 2) ────────────────────
        # Detach is NOT used here — we want the cls gradient to flow back
        # through cell_context2 and the attention layers, shaping those
        # representations to encode sensitivity.
...
def combined_loss(
    ...
    The auxiliary classification loss (Change 2) is computed separately in the
    training script and added on top, so this function is unchanged and all
...
def evaluate(
    ...
    cl_mean, cl_std,
    cls_auc        ← new: AUROC for sensitivity classification (approx, via rank corr)
    """
    model.eval()
    ...
    # ── Sensitivity classification AUROC (approx via rank correlation) ────
    # Full AUROC requires sorting; Wilcoxon rank-sum is exact but expensive.
    # We use a fast approximation: Pearson between sigmoid(logit) and label,
    # which correlates well with AUROC and requires no external dependency.
    sens_prob = torch.sigmoid(sens_logit)
    sp = sens_prob - sens_prob.mean()
    sl = sens_label - sens_label.mean()
    denom = sp.norm() * sl.norm()
    cls_auc_approx = ((sp * sl).sum() / (denom + 1e-8)).item()
