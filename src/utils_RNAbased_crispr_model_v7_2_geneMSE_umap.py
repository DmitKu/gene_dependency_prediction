...
===============
CRISPR Sensitivity Model v3 — Cross-Attention + Linear Bypass + UMAP Injection
...
--------------------
  ...
  CellTokenizer internal flow:
    cell_feat  →  compress  →  z              [B, compress_dim]
    gene_emb   →  film(z)   →  z              [B, compress_dim]  (gene conditioning)
    umap_coords→  umap_proj →  u              [B, compress_dim]  (UMAP supplement)
    z          = z + gate * u                 (gated additive injection)
    z          →  tokenize  →  cell_tokens    [B, n_slots, gene_hidden]
...
UMAP Injection Design
---------------------
  UMAP coordinates encode the cell-line's position in population topology space.
  They are injected after FiLM (gene conditioning), not before, so the ordering is:
    1. compress:  morphology (2388 features → compress_dim)
    2. film:      gene context shapes the morphological representation
    3. umap_proj: population-level position supplements the conditioned representation

  The injection is gated (sigmoid gate initialised to -2 → ~0.12 at t=0) so UMAP
  starts as a gentle nudge and the gate learns how much weight it deserves.

  UMAP features are loaded from /cl_umap/features in the HDF5 file and returned as
  position 8 of each __getitem__ tuple.
...
---------------------------------
  ...
  This weight is the 8th element (index 7) of each __getitem__ tuple and is passed
  ...
  ---------------------------------
      gene_feat, cell_feat, target, cl_idx, _, _, _, gene_var_w, umap_coords = batch
      loss, ... = combined_loss(pred, target, cl_idx,
                                gene_var_weight=gene_var_w, ...)

__getitem__ return order
------------------------
  ...
  [8] umap_coords     : Tensor [U]              ← NEW
  [9] gene_int_idx    : LongTensor scalar — integer gene index
"""
...
class GeneDataset(Dataset):
    ...
    cl_umap/features               : float32 array [n_cell_lines, U]   ← NEW
    cl_umap/model_id               : bytes array   [n_cell_lines]       ← NEW
    ...
    def __init__(
        ...
        with h5py.File(h5_path, "r") as f:
            ...
            # ── UMAP features ─────────────────────────────────────────────────
            # cl_umap/model_id must be aligned with cell_lines/model_id so the
            # same cl_model_id_to_index lookup works for both feature sets.
            umap_features_np  = f["cl_umap/features"][:]
            umap_model_ids    = f["cl_umap/model_id"][:]
        ...
        # ── Cell-line lookup (morphological features) ────────────────────────
        ...
        # ── UMAP lookup ───────────────────────────────────────────────────────
        # Build a separate index for UMAP model_ids in case ordering differs,
        # then reorder umap_features to match cell_lines ordering so we can
        # use the same cl_indices tensor for both arrays.
        umap_mid_to_idx = {mid.decode(): i for i, mid in enumerate(umap_model_ids)}

        # Reorder UMAP rows to match cell_lines/model_id ordering
        n_cl              = len(cl_model_ids)
        umap_dim          = umap_features_np.shape[1]
        umap_aligned      = np.zeros((n_cl, umap_dim), dtype=np.float32)
        for cl_mid, cl_i in self.cl_model_id_to_index.items():
            umap_i = umap_mid_to_idx.get(cl_mid)
            if umap_i is None:
                # If a cell line has no UMAP entry, leave as zeros
                pass
            else:
                umap_aligned[cl_i] = umap_features_np[umap_i]

        self.umap_features = torch.tensor(umap_aligned, dtype=torch.float32)
        ...
        print(
            ...
            f"  -> umap_features    shape={list(self.umap_features.shape)}"
        )
    ...
    def __getitem__(self, idx):
        ...
        return (
            ...
            self.umap_features[cl_i],              # [8] Tensor [U]   ← NEW
            self.gene_int_idx[idx],                # [9] LongTensor scalar — integer gene index
        )
...
class CellTokenizer(nn.Module):
    """
    Tokenizes cell-line features into a sequence of slot embeddings.

    UMAP injection flow
    -------------------
      z = compress(cell_feat)          # [B, compress_dim]   morphology
      z = film(z, gene_emb)            # gene context first
      u = umap_proj(umap_coords)       # [B, compress_dim]   population topology
      z = z + sigmoid(umap_gate) * u   # gated additive injection after FiLM
      tokens = tokenize(z) ...

    The gate is initialised to -2 so sigmoid(-2) ≈ 0.12 at t=0, keeping the
    UMAP signal as a gentle supplement that the network can grow or suppress.
    """
    ...
    def __init__(
        ...
        # ── UMAP injection (after FiLM) ──────────────────────────────────────
        # Bottleneck: umap_dim (typically 2–10) → 64 → compress_dim.
        # Direct Linear(2, 1024) has very few inputs driving many outputs and
        # tends toward degenerate solutions; the bottleneck avoids this.
        umap_bottleneck = min(64, compress_dim)
        self.umap_proj = nn.Sequential(
            nn.Linear(umap_dim, umap_bottleneck),
            nn.LayerNorm(umap_bottleneck),
            nn.GELU(),
            nn.Linear(umap_bottleneck, compress_dim),
        )
        # Gate initialised small so UMAP starts as a gentle nudge
        self.umap_gate = nn.Parameter(torch.tensor(-2.0))
    ...
    def forward(
        self,
        x:           torch.Tensor,   # [B, cell_feat_dim]
        gene_emb:    torch.Tensor,   # [B, gene_dim]
        umap_coords: torch.Tensor,   # [B, umap_dim]
    ) -> torch.Tensor:               # [B, n_slots, d_model]
        B = x.size(0)

        # 1. Compress morphological features
        z = self.compress(x)                          # [B, compress_dim]

        # 2. Gene conditioning (FiLM)
        z = self.film(z, gene_emb)                    # [B, compress_dim]

        # 3. Gated UMAP injection — after gene conditioning
        u    = self.umap_proj(umap_coords)            # [B, compress_dim]
        gate = torch.sigmoid(self.umap_gate)          # scalar ∈ (0, 1)
        z    = z + gate * u                           # [B, compress_dim]
...
class CRISPRSensitivityModelV3(nn.Module):
    """
    CRISPR sensitivity predictor — v3 + UMAP injection.

    New parameter vs previous version
    -----------------------------------
    umap_dim : int
        Dimensionality of the UMAP coordinates from /cl_umap/features.
        Passed through to CellTokenizer.  Default 2.
    """
    ...
    def forward(
        self,
        cell_features: torch.Tensor,   # [B, cell_features_size]
        gene_features: torch.Tensor,   # [B, gene_features_size]
        umap_coords:   torch.Tensor,   # [B, umap_dim]            ← NEW
        ablate_bypass: bool = False,
    ) -> tuple[torch.Tensor, torch.Tensor]:

        gene_emb    = self.gene_res(self.gene_encoder(gene_features))
        cell_tokens = self.cell_tokenizer(cell_features, gene_emb, umap_coords)  # ← passes umap
...
def gene_demeaned_mse(
    ...
    gene_idx: torch.Tensor,   # [B] integer gene index
    weights:  torch.Tensor = None,  # [B] optional, same weights as MSE term
) -> torch.Tensor:
    """
    ...
    # Average MSE per gene, then average across genes
    # Gives equal gradient weight regardless of how many cell lines
    # appear for each gene in this batch
    ...
    # Only genes with >=2 cell lines contribute any within-gene signal
    present = (counts > 1)
...
def combined_loss(
    ...
    """
    Combined MSE + Pearson + demeaned-Pearson loss.

    gene_var_weight : Tensor [B], optional
        Per-sample biological variance weight from GeneDataset (position 7
        of the batch tuple).  Multiplied with the deviation-based weight:

            final_weight = deviation_weight * gene_var_weight

        Both are renormalised together so loss magnitude stays stable.
        Pass None to disable (falls back to original deviation-only weighting).
    """
    pred_s   = pred.view(-1)
...
def evaluate(
    ...
    Unpacks position 7 (gene_var_weight) and position 8 (umap_coords)
    from the batch tuple.
    """
    model.eval()
    ...
    with torch.no_grad():
        for gene_feat, cell_feat, target, cl_idx, _, _, _, gene_var_w, umap_coords, gene_int_idx in loader:
            ...
            umap_coords = umap_coords.to(device, non_blocking=True)
            gene_int_idx = gene_int_idx.to(device, non_blocking=True)

            with autocast(device_type=device_type):
                pred, _ = model(cell_feat, gene_feat, umap_coords,
    ...
    # Gene-demeaned Pearson — demean by gene (not cell line)
    # This matches what the training loss optimises
    n_genes  = gene_int_idx.max() + 1      # gene_int_idx unpacked from batch pos [9]
    ...
    pred_gdm   = eval_pred   - t_gmeans    # pred relative to per-gene target mean
    target_gdm = eval_target - t_gmeans    # target centered per gene
    
    # NO second global mean subtraction — that was the bug
    pm_gd = pred_gdm
    pt_gd = target_gdm
    pearson_full_demeaned = (
        (pm_gd * pt_gd).sum() / (pm_gd.norm() * pt_gd.norm() + 1e-8)
    ).item()
...
def diagnose_bypass(
    ...
    Also prints the learned UMAP gate value for monitoring.
    """
    model.eval()
    bypass_mags, head_mags, attn_entropies = [], [], []

    with torch.no_grad():
        for i, (gene_feat, cell_feat, target, cl_idx, gene_id, model_id, _, _, umap_coords, gene_int_idx) in enumerate(loader):
            if i >= n_batches:
                break
            gene_feat   = gene_feat.to(device)
            cell_feat   = cell_feat.to(device)
            umap_coords = umap_coords.to(device)

            gene_emb    = model.gene_res(model.gene_encoder(gene_feat))
            cell_tokens = model.cell_tokenizer(cell_feat, gene_emb, umap_coords)
    ...
    umap_gate_val = torch.sigmoid(model.cell_tokenizer.umap_gate).item()
    ...
    print(f"  UMAP gate         : {umap_gate_val:.4f}  (init≈0.12  saturated: >0.9)")
    print(f"----------------------------------------------------------------\n")