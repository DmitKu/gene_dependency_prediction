...
def evaluate(
    ...
    with torch.no_grad():
        for gene_feat, cell_feat, target, cl_idx, _, _, _, gene_var_w, umap_coords in loader:
...
def diagnose_bypass(
    ...
    with torch.no_grad():
        for i, (gene_feat, cell_feat, target, cl_idx, gene_id, model_id, _, _, umap_coords) in enumerate(loader):
            if i >= n_batches:
                break
