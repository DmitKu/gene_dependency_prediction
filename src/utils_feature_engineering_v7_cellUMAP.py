...
# ══════════════════════════════════════════════════════════════════════════════
# Umap based CL features and visualization
# ══════════════════════════════════════════════════════════════════════════════

def train_and_apply_umap(full_df,
                         out_path,
                         n_neighbors=3, 
                         min_dist=0.05,
                         n_components=10,
                         exclude_cols = ['ModelID', 'split'],
                         ):
    """
    Trains UMAP on train_df and transforms the full_df based on that mapping.
    
    Parameters:
    - train_df: DataFrame used to fit the UMAP model.
    - full_df: DataFrame to transform using the fitted model.
    - n_neighbors: How many neighbors to consider for local structure.
    ...
    reducer_CL.fit(full_df.loc[full_df['split']=='train', feature_cols ])
    
    
    if out_path is not None:
        joblib.dump(reducer_CL, out_path)
        print(f"Model saved to {out_path}")
    # 3. Transform the full dataset
    # This maps the full_df into the same embedding space as train_df
    embedding = reducer_CL.transform(full_df[feature_cols])
    
    # DataFrame for easier handling
    CL_UMAP = pd.DataFrame(
                    embedding, 
                    columns=[f'umap_{i+1}' for i in range(n_components)]
                )
    CL_UMAP[['ModelID','split']] = full_df[['ModelID','split']]
    
    return CL_UMAP

def plot_CL_umap(X_umap, meta_df,
                 save_dir: Path, 
                 color_col: str = "OncotreePrimaryDisease", 
                 tag: str = "") -> None:
    X_umap_mrg = X_umap.merge(meta_df, on = 'ModelID')
    fig = px.scatter(
        data_frame=X_umap_mrg, 
        x='umap_1',            # Use the actual column names
        y='umap_2', 
        color=color_col,
    ...
    fname = f"CL_umap{'_' + tag if tag else ''}_{color_col}.html"
