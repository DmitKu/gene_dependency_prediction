...
class GeneDataset(Dataset):
    ...
    def _compute_gene_var_weights(
        ...
        """
        # Group scores by gene_id across the full HDF5 array
    ...
    def __getitem__(self, idx):
        return (
            ...
            self.cl_indices[idx],           # LongTensor scalar
            self.gene_ids[idx],             # str
            self.model_ids[idx],            # str
            idx,                            # int
            self.gene_var_weights[idx],     # float32 scalar Tensor  [NEW pos 7]
        )
...
class LinearBypass(nn.Module):
    def __init__(self, gene_dim: int, cell_dim: int, rank: int = 32, reg_scale: float = 1e-3):  #V4 was float = 1e-3
        super().__init__()
        self.gene_proj = nn.Linear(gene_dim, rank, bias=False)
        self.cell_proj = nn.Linear(cell_dim, rank, bias=False)
        self.scale     = nn.Parameter(torch.ones(1) * 0.5)
        self.gate      = nn.Parameter(torch.tensor(-2.0))  #V4 was -2.0; sigmoid(-1)≈0.27
...
def combined_loss(
    ...
    alpha:           float             = 0.5,
    beta:            float             = 0.4,
    gene_var_weight: torch.Tensor = None,    # ← NEW [B] float32
    ...
    The two weighting mechanisms are complementary:
      deviation_weight  — upweights extreme *values* within the current batch
      gene_var_weight   — upweights biologically *variable genes* globally
    """
    pred_s   = pred.view(-1)
    target_s = target.view(-1)

    with torch.no_grad():
        # Original deviation-based weight
        target_mean = target_s.mean()
        deviation   = (target_s - target_mean).abs()
        deviation_w = 1.0 + 2.0 * (deviation / (deviation.max() + 1e-8))

        # Multiply by gene variance weight when provided
        ...
        # Renormalise — keeps MSE magnitude identical to the no-weight baseline
...
def evaluate(
    ...
    with torch.no_grad():
        for gene_feat, cell_feat, target, cl_idx, _, _, _, gene_var_w in loader:
...
def diagnose_bypass(
    ...
    with torch.no_grad():
        for i, (gene_feat, cell_feat, target, cl_idx, gene_id, model_id, _, _) in enumerate(loader):
            if i >= n_batches:
                break
